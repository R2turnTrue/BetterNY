...
const addChatMessageClient = (data, options) => {
  if (data.username !== "" && data.message.length <= 250 && data.username.length <= 50) {
    ...
    var typingClass = data.typing ? "typing" : ""; // BetterNY #0: YT Embed

    let $messageDiv;

    if (data.message.startsWith("https://www.youtube.com/watch?v=")) {
      $messageDiv = $('<li class="message"/>').data("username", data.username).addClass(typingClass).append($usernameDiv, $messageBodyDiv, $("<br/>"), $(`<iframe width="560" height="315" src="https://www.youtube.com/embed/${data.message.replace("https://www.youtube.com/watch?v=", "")}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""/>`));
    } else if (data.message.startsWith("https://youtu.be")) {
      $messageDiv = $('<li class="message"/>').data("username", data.username).addClass(typingClass).append($usernameDiv, $messageBodyDiv, $("<br/>"), $(`<iframe width="560" height="315" src="https://www.youtube.com/embed/${data.message.replace("https://youtu.be/", "")}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""/>`));
    } // 취소선
